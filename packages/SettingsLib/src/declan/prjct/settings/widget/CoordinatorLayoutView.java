...
public class CoordinatorLayoutView extends CoordinatorLayout {
	...
	private void updateSettings() {
		...
		mDualToneMode = Settings.System.getIntForUser(cr, "declan_base_dual_tone_rounded", 0, UserHandle.USER_CURRENT) == 1;
		mCollapsedTitleMode = Settings.System.getIntForUser(cr, "declan_base_title_collapsed_mode", 0, UserHandle.USER_CURRENT);
		mTitleGravity = Settings.System.getIntForUser(cr, "declan_base_expanded_title_gravity", 0, UserHandle.USER_CURRENT);
		updateDualTone();
		updateTitleView();
	}
	...
	private void updateDualTone() {
		...
		scrimDrawable.setColor(DeclanUtils.getColorAttr(mContext, com.android.internal.R.attr.colorSurfaceHeader));
		scrimDrawable.setCornerRadii(new float[]{0, 0, 0, 0, radius, radius, radius, radius});
		if (mDualToneMode) {
			appBarDrawable.setColor(DeclanUtils.getColorAttr(mContext, com.android.internal.R.attr.colorSurfaceHeader));
			appBarDrawable.setCornerRadii(new float[]{0, 0, 0, 0, radius, radius, radius, radius});
		} else {
			appBarDrawable.setColor(DeclanUtils.getColorAttr(mContext, android.R.attr.colorBackground));
